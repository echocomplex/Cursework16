...
std::pair<List <std::pair<Student, float>>, List <std::pair<Student, float>>> University::sort (const std::string group) {
    List <std::pair<Student, float>> first, second;
    std::string firstPart = "ABCDEFGHIJKLMNO";
    for (int i = 0; i < this->database.length(); ++i) {
        if (this->database[i].getGroup() == group) {
            Student unit = this->database[i];
            float avg = 0;
            for (int session = 0; session < unit.getSessions().length(); ++session) {
                float localAvg = 0;
                for (int j = 0; j < unit.getSessions()[session].length(); ++j) {
                    localAvg += unit.getSessions()[session][j].mark;
                }
                localAvg /= unit.getSessions()[session].length();
                avg += localAvg;
            }
            avg /= unit.getSessions().length();
            if (firstPart.find(unit.getLastName()[0]) != std::string::npos) {
                first.append(std::make_pair(unit, avg));
            }
            else {
                second.append(std::make_pair(unit, avg));
            }
        }
    }
    if (first.length() > 0) {
        for (int i = 0; i < first.length() - 1; ++i) {
            for (int j = 0; j < first.length() - i - 1; ++j) {
                if (first[j].second < first[j + 1].second) {
                    std::pair<Student, float> temp = first[j];
                    first[j] = first[j + 1];
                    first[j + 1] = temp;
                }
            }
    ...
    if (second.length() > 0) {
        for (int i = 0; i < second.length() - 1; ++i) {
            for (int j = 0; j < second.length() - i - 1; ++j) {
                if (second[j].second < second[j + 1].second) {
                    std::pair<Student, float> temp = second[j];
                    second[j] = second[j + 1];
                    second[j + 1] = temp;
                }
            }
