...
void sortGroup (University& unit) {
    ...
    std::cin >> group;
    std::pair<List <std::pair<Student, float>>, List <std::pair<Student, float>>> result = unit.sort(group);
    if ((result.first.length() == 0) && (result.second.length())) {
        throw std::logic_error("Group doesn't exist!");
    }
    std::cout << "Part 1:" << std::endl;
    for (int i = 0; i < result.first.length(); ++i) {
        std::cout << result.first[i].first.getNumber() << " " << result.first[i].first.getLastName() << " >>> " << result.first[i].second << std::endl;
    }
    std::cout << std::endl << "Part 2:" << std::endl;
    for (int i = 0; i < result.second.length(); ++i) {
        std::cout << result.second[i].first.getNumber() << " " << result.second[i].first.getLastName() << " >>> " << result.second[i].second << std::endl;
    }
}
